...
-- Command line: lua\!InfrastructureShared\1f4b33781eaac\1.luac 
            ...
            if (not (this_sigattrlog[6]).matched or not (this_sigattrlog[6]).utf8p2 or (this_sigattrlog[7]).matched) and (this_sigattrlog[7]).utf8p2 then
              ...
              do
                ...
                do
                  ...
                  if contains(R15_PC363, l_0_22) and (contains(R15_PC363, {"iex", "iwr", "http", "irm", "curl", "invoke-restmethod", "invoke-webrequest", "invoke-expression", "windowsinstaller.installer", "start-process", "verif", " ray", " recaptcha", " re captcha", " rcaptcha", "press enter", " clip free", " over free", "robot: r", "robot - r", "robot - cloudflare", "robot: cloudflare", "robot: captcha", "robot - captcha", "human - r", "human: r", "human: captcha", "human - captcha", "microsoft windows: fix internet dns service reconnect", "restart dns service in the microsoft windows system", "# ", "identificate"}) or (MpCommon.StringRegExpSearch)("(?i)[^\\w\\d\\s]w[indowstyle]*\\s+(?:h[iden]*|1|m[inimized]*)\\s+", l_0_7)) then
                    return mp.INFECTED
                  end
                  return mp.CLEAN
                end
