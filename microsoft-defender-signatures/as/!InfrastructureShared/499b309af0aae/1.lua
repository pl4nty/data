-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.1 from https://github.com/viruscamp/luadec
-- Command line: lua\!InfrastructureShared\499b309af0aae\1.luac 

-- params : ...
-- function num : 0
if not IsDeviceHVA() then
  return mp.CLEAN
end
local l_0_0, l_0_1 = (bm.get_process_relationships)()
if l_0_1 == nil then
  return mp.CLEAN
end
local l_0_2 = {}
-- DECOMPILER ERROR at PC35: No list found for R2 , SetList fails

-- DECOMPILER ERROR at PC36: Overwrote pending register: R3 in 'AssignReg'

local l_0_3 = "whoami.exe"
local l_0_4 = {}
-- DECOMPILER ERROR at PC38: Overwrote pending register: R5 in 'AssignReg'

local l_0_5 = "systeminfo.exe"
-- DECOMPILER ERROR at PC39: Overwrote pending register: R6 in 'AssignReg'

-- DECOMPILER ERROR at PC40: Overwrote pending register: R7 in 'AssignReg'

for l_0_9,l_0_10 in ("hostname.exe")("nltest.exe") do
  -- DECOMPILER ERROR at PC43: Overwrote pending register: R11 in 'AssignReg'

  -- DECOMPILER ERROR at PC51: Overwrote pending register: R12 in 'AssignReg'

  -- DECOMPILER ERROR at PC52: Overwrote pending register: R13 in 'AssignReg'

  if "query.exe" ~= nil and l_0_10.ppid ~= nil then
    local l_0_11 = (mp.bitand)("tasklist.exe", "arp.exe")
    if l_0_11 == l_0_5 then
      local l_0_12 = (string.lower)(l_0_10.image_path)
      -- DECOMPILER ERROR at PC61: Overwrote pending register: R14 in 'AssignReg'

      for l_0_16,l_0_17 in ipairs("nslookup.exe") do
        -- DECOMPILER ERROR at PC64: Overwrote pending register: R18 in 'AssignReg'

        -- DECOMPILER ERROR at PC66: Overwrote pending register: R19 in 'AssignReg'

        -- DECOMPILER ERROR at PC67: Overwrote pending register: R20 in 'AssignReg'

        -- DECOMPILER ERROR at PC68: Overwrote pending register: R21 in 'AssignReg'

        if (("route.exe").find)("wmic.exe", "cmdkey.exe", "klist.exe", true) then
          do
            do
              local l_0_18 = #l_0_4 + 1
              do break end
              -- DECOMPILER ERROR at PC85: LeaveBlock: unexpected jumping out DO_STMT

              -- DECOMPILER ERROR at PC85: LeaveBlock: unexpected jumping out IF_THEN_STMT

              -- DECOMPILER ERROR at PC85: LeaveBlock: unexpected jumping out IF_STMT

            end
          end
        end
      end
      if (string.find)(l_0_12, "\\cmd.exe", 1, true) and l_0_10.cmd_line then
        local l_0_19 = (string.lower)(l_0_10.cmd_line)
        -- DECOMPILER ERROR at PC175: Overwrote pending register: R3 in 'AssignReg'

        if (string.find)(l_0_19, "whoami", 1, true) or (string.find)(l_0_19, "ipconfig", 1, true) or (string.find)(l_0_19, "net user", 1, true) or (string.find)(l_0_19, "net group", 1, true) or (string.find)(l_0_19, "net localgroup", 1, true) or (string.find)(l_0_19, "systeminfo", 1, true) or (string.find)(l_0_19, "tasklist", 1, true) or (string.find)(l_0_19, "nltest", 1, true) then
          local l_0_20 = #l_0_4 + 1
        end
      end
    end
  end
end
if l_0_3 < 2 then
  return mp.CLEAN
end
local l_0_21 = (table.concat)(l_0_4, "|")
if #l_0_21 > 2000 then
  l_0_21 = (string.sub)(l_0_21, 1, 2000)
end
;
(bm.add_related_string)("WinRMRecon_count", tostring(l_0_3), bm.RelatedStringBMReport)
;
(bm.add_related_string)("WinRMRecon_cmds", l_0_21, bm.RelatedStringBMReport)
TrackPidAndTechniqueBM("BM", "T1021.006", "WinRMRecon")
add_parents()
return mp.INFECTED

