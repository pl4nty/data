-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.1 from https://github.com/viruscamp/luadec
-- Command line: lua\!InfrastructureShared\!#SLFTrojanWin32\1.luac 

-- params : ...
-- function num : 0
if (mp.get_contextdata)(mp.CONTEXT_DATA_SCANREASON) ~= mp.SCANREASON_ONMODIFIEDHANDLECLOSE or not (mp.get_contextdata)(mp.CONTEXT_DATA_NEWLYCREATEDHINT) then
  return mp.CLEAN
end
local l_0_0 = (MpCommon.ExpandEnvironmentVariables)("%ExchangeInstallPath%")
if not isnull(l_0_0) and l_0_0 ~= "%ExchangeInstallPath%" and (sysio.IsFileExists)(l_0_0 .. "bin\\Microsoft.Exchange.Store.Service.exe") then
  return mp.CLEAN
end
local l_0_1 = (mp.get_contextdata)(mp.CONTEXT_DATA_FILENAME)
if isnull(l_0_1) then
  return mp.CLEAN
end
local l_0_2 = (string.match)(l_0_1, "(%.[^%.]+)$")
if isnull(l_0_2) then
  return mp.CLEAN
end
l_0_2 = (string.lower)(l_0_2)
if l_0_2 ~= ".asp" and l_0_2 ~= ".aspx" and l_0_2 ~= ".ashx" then
  return mp.CLEAN
end
local l_0_3 = (MpCommon.PathToWin32Path)((mp.getfilename)((mp.bitor)(mp.FILEPATH_QUERY_FULL, mp.FILEPATH_QUERY_LOWERCASE)))
if isnull(l_0_3) then
  return mp.CLEAN
end
l_0_3 = (string.lower)(l_0_3)
local l_0_4 = (MpCommon.ExpandEnvironmentVariables)("%SystemDrive%\\inetpub\\wwwroot")
if not isnull(l_0_4) and (string.find)(l_0_3, (string.lower)(l_0_4), 1, true) then
  return mp.INFECTED
end
local l_0_5 = (MpCommon.ExpandEnvironmentVariables)("%windir%\\system32\\inetsrv\\config\\applicationHost.config")
if isnull(l_0_5) then
  return mp.CLEAN
end
local l_0_6 = (MpCommon.GetIisInstallPaths)(l_0_5)
if isnull(l_0_6) then
  return mp.CLEAN
end
for l_0_10,l_0_11 in ipairs(l_0_6) do
  if not isnull(l_0_11.Path) and (string.find)(l_0_3, (string.lower)(l_0_11.Path), 1, true) then
    return mp.INFECTED
  end
end
return mp.CLEAN

