...
-- Command line: lua\!InfrastructureShared\219d75855d29d\1.luac 
...
do
  ...
  do
    ...
    if not ({["cmd.exe"] = true, ["powershell.exe"] = true, ["pwsh.exe"] = true, ["wscript.exe"] = true, ["cscript.exe"] = true, ["mshta.exe"] = true, ["wmic.exe"] = true})[((string.lower)(((mp.GetParentProcInfo)()).image_path)):match("([^\\]+)$")] then
      return mp.CLEAN
    end
