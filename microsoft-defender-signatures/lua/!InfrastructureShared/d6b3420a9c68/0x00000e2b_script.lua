...
-- Command line: lua\!InfrastructureShared\d6b3420a9c68\0x00000e2b_luac 
...
do
  if ((((this_sigattrlog[2]).matched and (this_sigattrlog[2]).utf8p2) or ((this_sigattrlog[3]).matched and (this_sigattrlog[3]).utf8p2) or ((this_sigattrlog[4]).matched and (this_sigattrlog[4]).utf8p2) or (this_sigattrlog[5]).matched)) then
    local l_0_0, l_0_1 = (this_sigattrlog[5]).utf8p2
  end
  ...
  if l_0_0 then
    ...
    if not isValidLocalDomain((string.match)(l_0_0, "Uri=([^;]+)")) then
      ...
      if (string.match)(l_0_3, "^https?://") then
        local l_0_6 = {SIG_CONTEXT = "BM", CONTENT_SOURCE = "ZERODAY-OBVIOUSLY", PROCESS_CONTEXT = "PE", FILELESS = "true", CMDLINE_URL = "true"}
