-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.1 from https://github.com/viruscamp/luadec
-- Command line: lua\!InfrastructureShared\425b32660ac48\0x00000e6b_luac 

-- params : ...
-- function num : 0
local l_0_0 = ((bm.get_imagepath)()):lower()
local l_0_1 = contains
local l_0_2 = l_0_0
local l_0_3 = {}
-- DECOMPILER ERROR at PC10: No list found for R3 , SetList fails

-- DECOMPILER ERROR at PC14: Overwrote pending register: R1 in 'AssignReg'

-- DECOMPILER ERROR at PC15: Overwrote pending register: R1 in 'AssignReg'

if l_0_1 then
  return l_0_1
end
-- DECOMPILER ERROR at PC17: Overwrote pending register: R1 in 'AssignReg'

-- DECOMPILER ERROR at PC20: Overwrote pending register: R3 in 'AssignReg'

-- DECOMPILER ERROR at PC21: Overwrote pending register: R3 in 'AssignReg'

-- DECOMPILER ERROR at PC24: Overwrote pending register: R3 in 'AssignReg'

-- DECOMPILER ERROR at PC25: Overwrote pending register: R3 in 'AssignReg'

-- DECOMPILER ERROR at PC26: Overwrote pending register: R1 in 'AssignReg'

-- DECOMPILER ERROR at PC27: Overwrote pending register: R3 in 'AssignReg'

-- DECOMPILER ERROR at PC28: Overwrote pending register: R3 in 'AssignReg'

-- DECOMPILER ERROR at PC29: Overwrote pending register: R4 in 'AssignReg'

-- DECOMPILER ERROR at PC30: Overwrote pending register: R5 in 'AssignReg'

if l_0_3 then
  l_0_3 = l_0_3("conhost.exe", "WerFault.exe")
  if not l_0_3 then
    l_0_3 = mp
    l_0_3 = l_0_3.CLEAN
    return l_0_3
  end
  -- DECOMPILER ERROR at PC37: Overwrote pending register: R2 in 'AssignReg'

else
  l_0_3 = this_sigattrlog
  l_0_3 = l_0_3[3]
  l_0_3 = l_0_3.matched
  if l_0_3 then
    l_0_3 = this_sigattrlog
    l_0_3 = l_0_3[3]
    -- DECOMPILER ERROR at PC46: Overwrote pending register: R1 in 'AssignReg'

    -- DECOMPILER ERROR at PC47: Overwrote pending register: R2 in 'AssignReg'

  else
    l_0_3 = this_sigattrlog
    l_0_3 = l_0_3[4]
    l_0_3 = l_0_3.matched
    if l_0_3 then
      l_0_3 = this_sigattrlog
      l_0_3 = l_0_3[4]
      -- DECOMPILER ERROR at PC56: Overwrote pending register: R1 in 'AssignReg'

      -- DECOMPILER ERROR at PC57: Overwrote pending register: R2 in 'AssignReg'

    else
      l_0_3 = this_sigattrlog
      l_0_3 = l_0_3[5]
      l_0_3 = l_0_3.matched
      if l_0_3 then
        l_0_3 = this_sigattrlog
        l_0_3 = l_0_3[5]
        -- DECOMPILER ERROR at PC66: Overwrote pending register: R1 in 'AssignReg'

        -- DECOMPILER ERROR at PC67: Overwrote pending register: R2 in 'AssignReg'

      else
        l_0_3 = this_sigattrlog
        l_0_3 = l_0_3[6]
        l_0_3 = l_0_3.matched
        if l_0_3 then
          l_0_3 = this_sigattrlog
          l_0_3 = l_0_3[6]
          -- DECOMPILER ERROR at PC76: Overwrote pending register: R1 in 'AssignReg'

          -- DECOMPILER ERROR at PC77: Overwrote pending register: R2 in 'AssignReg'

        else
          l_0_3 = this_sigattrlog
          l_0_3 = l_0_3[7]
          l_0_3 = l_0_3.matched
          if l_0_3 then
            l_0_3 = this_sigattrlog
            l_0_3 = l_0_3[7]
            -- DECOMPILER ERROR at PC86: Overwrote pending register: R1 in 'AssignReg'

            -- DECOMPILER ERROR at PC87: Overwrote pending register: R2 in 'AssignReg'

          else
            l_0_3 = this_sigattrlog
            l_0_3 = l_0_3[8]
            l_0_3 = l_0_3.matched
            if l_0_3 then
              l_0_3 = this_sigattrlog
              l_0_3 = l_0_3[8]
              -- DECOMPILER ERROR at PC96: Overwrote pending register: R1 in 'AssignReg'

              -- DECOMPILER ERROR at PC97: Overwrote pending register: R2 in 'AssignReg'

            else
              l_0_3 = this_sigattrlog
              l_0_3 = l_0_3[9]
              l_0_3 = l_0_3.matched
              if l_0_3 then
                l_0_3 = this_sigattrlog
                l_0_3 = l_0_3[9]
                -- DECOMPILER ERROR at PC106: Overwrote pending register: R1 in 'AssignReg'

                -- DECOMPILER ERROR at PC107: Overwrote pending register: R2 in 'AssignReg'

              else
                l_0_3 = this_sigattrlog
                l_0_3 = l_0_3[10]
                l_0_3 = l_0_3.matched
                if l_0_3 then
                  l_0_3 = this_sigattrlog
                  l_0_3 = l_0_3[10]
                  -- DECOMPILER ERROR at PC116: Overwrote pending register: R1 in 'AssignReg'

                  -- DECOMPILER ERROR at PC117: Overwrote pending register: R2 in 'AssignReg'

                else
                  l_0_3 = this_sigattrlog
                  l_0_3 = l_0_3[2]
                  l_0_3 = l_0_3.matched
                  if l_0_3 then
                    l_0_3 = this_sigattrlog
                    l_0_3 = l_0_3[2]
                    -- DECOMPILER ERROR at PC126: Overwrote pending register: R1 in 'AssignReg'

                    l_0_3 = MpCommon
                    l_0_3 = l_0_3.StringRegExpSearch
                    l_0_3 = l_0_3("exec\\s+(puma|thin|rails|mongrel) ", l_0_1)
                    if not l_0_3 then
                      l_0_3 = mp
                      l_0_3 = l_0_3.CLEAN
                      return l_0_3
                    end
                  else
                    l_0_3 = this_sigattrlog
                    l_0_3 = l_0_3[11]
                    l_0_3 = l_0_3.matched
                    if l_0_3 then
                      l_0_3 = this_sigattrlog
                      l_0_3 = l_0_3[11]
                      -- DECOMPILER ERROR at PC145: Overwrote pending register: R1 in 'AssignReg'

                    else
                      l_0_3 = this_sigattrlog
                      l_0_3 = l_0_3[12]
                      l_0_3 = l_0_3.matched
                      if l_0_3 then
                        l_0_3 = this_sigattrlog
                        l_0_3 = l_0_3[12]
                        -- DECOMPILER ERROR at PC154: Overwrote pending register: R1 in 'AssignReg'

                      else
                        l_0_3 = this_sigattrlog
                        l_0_3 = l_0_3[13]
                        l_0_3 = l_0_3.matched
                        if l_0_3 then
                          l_0_3 = this_sigattrlog
                          l_0_3 = l_0_3[13]
                          -- DECOMPILER ERROR at PC163: Overwrote pending register: R1 in 'AssignReg'

                        else
                          l_0_3 = this_sigattrlog
                          l_0_3 = l_0_3[14]
                          l_0_3 = l_0_3.matched
                          if l_0_3 then
                            l_0_3 = this_sigattrlog
                            l_0_3 = l_0_3[14]
                            -- DECOMPILER ERROR at PC172: Overwrote pending register: R1 in 'AssignReg'

                          else
                            l_0_3 = this_sigattrlog
                            l_0_3 = l_0_3[15]
                            l_0_3 = l_0_3.matched
                            if l_0_3 then
                              l_0_3 = this_sigattrlog
                              l_0_3 = l_0_3[15]
                              -- DECOMPILER ERROR at PC181: Overwrote pending register: R1 in 'AssignReg'

                            else
                              l_0_3 = this_sigattrlog
                              l_0_3 = l_0_3[16]
                              l_0_3 = l_0_3.matched
                              if l_0_3 then
                                l_0_3 = this_sigattrlog
                                l_0_3 = l_0_3[16]
                                -- DECOMPILER ERROR at PC190: Overwrote pending register: R1 in 'AssignReg'

                              else
                                l_0_3 = this_sigattrlog
                                l_0_3 = l_0_3[17]
                                l_0_3 = l_0_3.matched
                                if l_0_3 then
                                  l_0_3 = this_sigattrlog
                                  l_0_3 = l_0_3[17]
                                  -- DECOMPILER ERROR at PC199: Overwrote pending register: R1 in 'AssignReg'

                                else
                                  l_0_3 = this_sigattrlog
                                  l_0_3 = l_0_3[18]
                                  l_0_3 = l_0_3.matched
                                  if l_0_3 then
                                    l_0_3 = this_sigattrlog
                                    l_0_3 = l_0_3[18]
                                    -- DECOMPILER ERROR at PC208: Overwrote pending register: R1 in 'AssignReg'

                                  end
                                end
                              end
                            end
                          end
                        end
                      end
                    end
                  end
                end
              end
            end
          end
        end
      end
    end
  end
end
if not l_0_1 then
  l_0_3 = mp
  l_0_3 = l_0_3.CLEAN
  return l_0_3
end
l_0_3 = l_0_3(l_0_1, "-a%s+([%d%.]+)")
local l_0_4 = l_0_1:match("-p%s+([%d]+)")
if not l_0_3 then
  return mp.CLEAN
end
if l_0_3 and (l_0_3 == "127.0.0.1" or l_0_4 == "0.0.0.0") then
  return mp.CLEAN
end
do
  local l_0_5 = l_0_3 .. ":" .. l_0_4 or "default"
  local l_0_6 = nil
  local l_0_7 = "ruby_webservers_config"
  local l_0_8 = 31536000
  AppendToRollingQueue(l_0_7, "IP/PORT", l_0_6, l_0_8, 50)
  local l_0_9 = nil
  -- DECOMPILER ERROR at PC257: Overwrote pending register: R9 in 'AssignReg'

  if l_0_2 then
    do
      local l_0_10 = nil
      if (bm.get_current_process_startup_info)() and ((bm.get_current_process_startup_info)()).ppid then
        l_0_10 = ((bm.get_current_process_startup_info)()).ppid
      end
      -- DECOMPILER ERROR at PC268: Confused about usage of register: R9 in 'UnsetPending'

      if not l_0_10 then
        return mp.CLEAN
      end
      local l_0_11 = nil
      if not (MpCommon.DoesProcessHaveAttribute)(l_0_11, "PFApp_Parent") then
        local l_0_12, l_0_13 = , pcall(MpCommon.AddProcessAttribute, l_0_11, "PFApp_Parent", safeJsonSerialize({AppName = "ruby.exe", WebAppName = getWebApplicationType(l_0_11), Pid = l_0_11, AppCategory = "PFServers", Score = 0}), true)
        do
          if not l_0_13 then
            local l_0_14 = nil
            ;
            (bm.trigger_sig)("ProcAttribError", safeJsonSerialize({err = l_0_14, AttributeValue = l_0_12, SignatureName = "SetProcAttribRBY.A", ProcessName = l_0_0}))
          end
          do
            if not l_0_2 then
              local l_0_15 = nil
              ;
              (bm.trigger_sig)("SetProcessAttrib", safeJsonSerialize({AttributeValue = l_0_12, SignatureName = "SetProcAttribRBY.A", ProcessName = l_0_0}))
            end
            -- DECOMPILER ERROR at PC330: Confused about usage of register: R10 in 'UnsetPending'

            ;
            (bm.trigger_sig)("SetProcessAttrib_RubyServr", safeJsonSerialize(l_0_12))
            return mp.CLEAN
          end
        end
      end
    end
  end
end

