...
class LuaFunc(object):
    ...
    def export(self, root=False):
        ...
        out.append(struct.pack("BBBB", self.nb_upvalues,
                   self.nb_params, self.is_vararg, self.max_stacksize))
...
with open(infile, 'r') as file:
    ...
    for row in csvreader:
        ...
        attr = row[1]
        category = cleanFilename(row[2])
        size = row[3]
        data = bytes.fromhex(row[4])
        key = f"{threat}-{category}"
        if (key in indexes):
            indexes[key] += 1
        else:
            indexes[key] = 1
...
print("Total: %d lua script" % (sum(indexes.values())))
