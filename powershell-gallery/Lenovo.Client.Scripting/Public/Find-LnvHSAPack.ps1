<#
  .SYNOPSIS
  Returns details about the available HSA packs for a specified machine type.

  .DESCRIPTION
  Returns a list of the available HSA packs for the machine type specified
  ...
  .PARAMETER Latest
  Switch to return only the latest HSA pack.

  .EXAMPLE
  Find-LnvHSAPack -MachineType 21DD

  .EXAMPLE
  Find-LnvHSAPack 21DD
...
function Find-LnvHSAPack
{
    ...
    try
    {
        ...
        # If HSA packs exist, return them or return the latest, if requested
        $hsaPacks = $node.HSA
        if ($null -eq $hsaPacks)
        {
            Write-Warning "No HSA packs found for machine type '$MachineType'."
            return
        }

        if ($Latest)
        {
            # Return only the latest HSA pack
            return $hsaPacks | Select-Object -Last 1
        }
        else
        {
            # Return all HSA packs for the machine type
            return $hsaPacks
        }
    }
    catch
    {
        Write-Error "An error occurred while retrieving HSA packs: $_"
        throw
    }
}
