...
{ 
	...
	memset(skb->tail, 0, tail_room_required); 
...
/*
 * N9 Component UID: a87d9f43-e240-4dab-8a85-54512ddffe00
 */
static const guid_t n9_guid = GUID_INIT(0xa87d9f43, 0xe240, 0x4dab,
		  0x8a, 0x85, 0x54, 0x51, 0x2d, 0xdf, 0xfe, 0x00);

void mt3620_hif_fw_download()
{
    fw_dl_data_t data;
	uint64_t wifi_flash_offset;
	uint64_t wifi_length;
	uint32_t wifi_flash_offset32;
	uint32_t wifi_length32;

	struct {
		uint32_t base;
		uint32_t length;
	} *orig_regs_from_devicetree;

	struct device_node *flash_node = NULL;
	struct property *flash_regs_property = NULL;

	uint32_t flash_base;

	// Find the offset into flash of the firmware
	int ret = azure_sphere_sm_get_image_details(&n9_guid, &wifi_flash_offset, &wifi_length); 
	if (ret != 0) {
		dev_err(g_pmt3620_hif_proc->dev,"<<%s>> Failed to get Wi-Fi firmware location. Error = %d.\n", __FUNCTION__, (int)ret);
		return;
	}

	wifi_flash_offset32 = (uint32_t)wifi_flash_offset;
	wifi_length32 = (uint32_t)wifi_length;

	// This only runs on MT3620 so we should never hit this, but check just in case
	if(wifi_flash_offset32 != wifi_flash_offset || wifi_length32 != wifi_length) {
		dev_err(g_pmt3620_hif_proc->dev,"<<%s>> Wifi location does not fit in 32 bits\n", __FUNCTION__);
		return;
	}

	// Find the /flash@xyz node in the device tree
	flash_node = of_parse_phandle(g_pmt3620_hif_proc->dev->of_node, "mediatek,xip-flash", 0 /*index*/);
	if (!flash_node) {
		dev_err(g_pmt3620_hif_proc->dev, "<<%s>> Failed to find flash\n", __FUNCTION__);
		return;
	}

	// Find the "reg" property, which has offset/length information.
	flash_regs_property = of_find_property(flash_node, "reg", NULL);
	if (!flash_regs_property) {
		dev_err(g_pmt3620_hif_proc->dev, "<<%s>> Failed to find flash property\n", __FUNCTION__);
	...
	// Convert flash base to appropriate endianness, and use it to map the memory.
	// This driver is 3620-specific, so it's reasonable to assume memory-mapped flash
	// is available.
	orig_regs_from_devicetree = flash_regs_property->value;
	flash_base = be32_to_cpu(orig_regs_from_devicetree->base);

	// Check overflow on offset type
	if(wifi_flash_offset32 + flash_base < wifi_flash_offset32) {
		dev_err(g_pmt3620_hif_proc->dev,"<<%s>> Wi-Fi firmware offset overflow.\n", __FUNCTION__);
		return;
	}

	data.image = ioremap(wifi_flash_offset32 + flash_base, wifi_length32);
	if (mt3620_hif_get_ilm_image_info_from_flash(&data, wifi_length32) != 0) {
