...
static void mt3620_wifi_scan_completed_event(struct mt3620_wifi_vif *vif)
{
    ...
    dev_info(vif->dev, "Connection state: %i", vif->state);
                     ...
                     void *data, u32 data_size)
{
    ...
    if (bss == NULL) {
        u8 ie_buf[IEEE80211_MAX_SSID_LEN + 2];
        ...
        if(vif->port == WIFI_PORT_IBSS){
            ...
            bss = cfg80211_inform_bss(wifi_hw->wifi->wiphy, channel,
                      ...
                      vif->mode_data.sta.ssid_len + 2, 0, GFP_KERNEL);
        }else{
            bss = cfg80211_inform_bss(wifi_hw->wifi->wiphy, channel,
                      CFG80211_BSS_FTYPE_UNKNOWN, bssid, 0,
    ...
    {
    if (is_roaming) {
        roam_info.channel = channel;
        roam_info.bssid = bssid;
        roam_info.req_ie = vif->mode_data.sta.req_ie;
        roam_info.req_ie_len = vif->mode_data.sta.req_ie_len;
        roam_info.resp_ie = vif->mode_data.sta.resp_ie;
        roam_info.resp_ie_len = vif->mode_data.sta.resp_ie_len;
                 ...
                 struct mt3620_wifi_vif *vif)
{
    ...
    // Patch configuration errata if needed
    // From observing the behavior of the RfToolCli, the hardware needs to be in STA mode,
    // which is the only mode it is operated in today, but to avoid being surprised if AP mode is
    // ever enabled as a feature, check anyway.

    if (!wifi_hw->efuses_checked && vif->port == WIFI_PORT_STA) {
        ret = mt3620_validate_and_fix_efuse_errata(wifi_hw->wifi->wiphy);
        if (ret != SUCCESS) {
            dev_err(wifi_hw->wifi->dev, "Unable to patch EFUSE errata %u", ret);
            goto exit;
        }

        wifi_hw->efuses_checked = true;
    }

    // Additionally, to avoid surprises in the future, warn if another mode is the first interface
    // to come up (before the e-fuses have had a chance to be checked).
    WARN_ON(vif->port != WIFI_PORT_STA && !wifi_hw->efuses_checked);
                   ...
                   struct cfg80211_connect_params *params)
{
    ...
    dev_info(vif->dev, "%s: Connection state: %i", __FUNCTION__, vif->state);
...
{
    int has_capability;
    has_capability = azure_sphere_has_pluton_capability(PLUTON_DEVICE_CAPABILITY_ENABLE_RF_TEST_MODE);
    if(has_capability == -1) {
        return -1;
    }
