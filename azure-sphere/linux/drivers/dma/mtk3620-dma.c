...
static inline struct mtk_desc *virt_dma_desc_to_mtk_dma_desc(struct virt_dma_desc *vdesc)
{
	return container_of(vdesc, struct mtk_desc, vd);
}
...
static void mtk_dma_desc_free(struct virt_dma_desc *vd)
{
	...
	struct mtk_desc *desc = virt_dma_desc_to_mtk_dma_desc(vd); 
	unsigned int count;
	...
	if(c->cfg.slave_id == SLAVE_ID_I2S || c->cfg.slave_id == SLAVE_ID_ADC){
		if (desc->control_block_cyclic != NULL) {
			for (count = 0; count < desc->sg_num; count++) {
				if (desc->control_block_cyclic[count].cb != NULL){
					kfree(desc->control_block_cyclic[count].cb);
					desc->control_block_cyclic[count].cb = NULL;
				}
			}

			desc->control_block_cyclic= NULL;
		}
	}else{
		if (desc->sg != NULL) {
			for (count = 0; count < desc->sg_num; count++) {
				if (desc->sg[count].cb != NULL){
					kfree(desc->sg[count].cb);
...
static void mtk_set_cyclic_dma_param(struct mtk_chan *c)
{
	...
	if (!c->desc || !c->desc->control_block_cyclic) {
		dev_err(c->vc.chan.device->dev, "\t[%s]\tnull mtk_chan pointer\n",
			__func__);
		return;
	}
...
{
	size_t size = 0;
	size = mtk_dma_chan_read(c, DMA_LEN1);
	return size;
}
	...
	unsigned int sg_len)
{
	...
	switch (c->cfg.slave_id) { /* slave id */
		...
		case SLAVE_ID_I2S:
			//max_len
			return 0;	//failure case as we should not have this slave ID
		case SLAVE_ID_ADC:
...
static int mtk_dma_probe(struct platform_device *pdev)
{
	...
	for(i = 0; i < (m2m_ch_num+peri_ch_num+vff_ch_num); i++){
		...
		if (of_property_read_string_index
			...
			strncpy(c->irq_name, s, sizeof(c->irq_name));
			c->irq_name[sizeof(c->irq_name) - 1] = '\0';
		}
	}
