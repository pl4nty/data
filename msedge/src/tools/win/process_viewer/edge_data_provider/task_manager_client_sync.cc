...
TaskManagerClientSync::~TaskManagerClientSync() {
  if (ipc_thread_) {
    // base::Unretained is safe: Stop() below drains all pending tasks before
    // returning, so task_manager_client_ is guaranteed alive when Disconnect
    // executes.
...
bool TaskManagerClientSync::Initialize() {
  ...
  // base::Unretained is safe: std::move below transfers ownership to
  // task_manager_client_ without changing the pointer value, so the object
  // remains valid when the posted task executes.
