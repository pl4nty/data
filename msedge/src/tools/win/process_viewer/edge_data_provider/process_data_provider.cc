...
#include "tools/win/process_viewer/edge_data_provider/edge_process.h"
#include "tools/win/process_viewer/edge_data_provider/edge_webview_process.h"
#include "tools/win/process_viewer/edge_data_provider/ie_process.h"
#include "tools/win/process_viewer/edge_data_provider/ntdefs.h"
...
void ProcessDataProvider::Refresh() {
  ...
  while (true) {
    ...
    if (process_type != ProcessViewType::Unknown) {
      ...
      LONGLONG new_process_cpu_time =
          system_process_info->UserTime + system_process_info->KernelTime;
      process->UpdateCpuUsage(new_process_cpu_time, system_cpu_time);
      process->UpdateMemoryUsage(system_process_info->WorkingSetPrivateSize);
      process->Refresh();
    }
