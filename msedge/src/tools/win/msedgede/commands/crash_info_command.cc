...
HRESULT CrashInfoCommand::Execute() {
  ...
  if (crashpad_dump) {
    ...
    hr = debug_client()->QueryInterface(IID_PPV_ARGS(&debug_advanced_));
    if (FAILED(hr)) {
...
void CrashInfoCommand::DisplayAnnotations() {
  ...
  // streams.
  std::unique_ptr<FileReaderInterface> dump_file_reader = OpenDumpFileReader();
  if (!dump_file_reader) {
      ...
      for (const AnnotationSnapshot& annotation : module->AnnotationObjects()) {
        ...
        if (annotation.type ==
          ...
          Printf("%s\n", value.c_str());
                   ...
                   static_cast<uint16_t>(Annotation::Type::kUserDefinedStart)) {
...
std::unique_ptr<FileReaderInterface> CrashInfoCommand::OpenDumpFileReader() {
  ULONG64 wide_dump_file_handle;
  ULONG dump_type;
  HRESULT hr = debug_client()->GetDumpFile(0, nullptr, 0, nullptr,
                                           &wide_dump_file_handle, &dump_type);
  if (FAILED(hr)) {
    PrintErrorf("getting dump file handle: %08x\n", hr);
    return nullptr;
  }

  FileHandle dump_file_handle = reinterpret_cast<FileHandle>(wide_dump_file_handle);
  return std::make_unique<WeakFileHandleFileReader>(dump_file_handle);
}
