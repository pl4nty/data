...
function install() {
  ...
  Object.defineProperty(
      window.__edgeTrackingPreventionStatistics, 'setEnabled', {
        enumerable: false,
        configurable: false,
        writable: false,
  ...
  function injectStatsTracking(enabled) {
    // This enable/disable section is a change from the original Focus iOS
    // version.
    if (enabled) {
      if (originalXHROpen) {
        return;
      ...
      if (originalXHROpen) {  // if one is set, then all the enable code has run
        ...
        originalXHROpen = originalXHRSend = originalImageSrc =
