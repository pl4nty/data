...
define(['lib/knockout', 'legacy/bridge', 'legacy/events', 'legacy/core'], (ko, bridge, constants, core) => {
    class OemRegistrationViewModel {
        constructor(resourceStrings, regions, defaultRegion, oemRegistrationInfo, userInfo, targetPersonality) {
            ...
            this.customerInfo = {
                firstName: {
                    ...
                    value: ko.observable(userInfo[0] || ""),
            ...
            // Call saveOEMInfo with empty Json object to delete any user info data saved on previous visit to the page.
            // The ViewModel is only constructed when the page is actually shown.
            var emptyObj = {};
            bridge.invoke("CloudExperienceHost.OemRegistration.saveOEMInfo", JSON.stringify(emptyObj)).then(() => {
                bridge.invoke("CloudExperienceHost.Telemetry.logEvent", "deleteOemRegistrationInfoSuccess");
                }, (error) => {
                bridge.invoke("CloudExperienceHost.Telemetry.logEvent", "deleteOemRegistrationInfoFailure", core.GetJsonFromError(error));
            });
        }
        ...
        saveInfoAsync() {
                ...
                Object.keys(this.customerInfo).forEach((key, index) => {
                    ...
                    if (key == "region") {
                        ...
                        if (field.value && field.value.countryCode && field.value.countryCode.rawCode && field.value.countryCode.rawCode.length > 0 && field.value.number && field.value.number.length > 0) {
                            ...
                            field.value = field.value.countryCode + " " + field.value.number;
                        }
            ...
            bridge.invoke("CloudExperienceHost.OemRegistration.saveOEMInfo", JSON.stringify(registrationInfo)).then(() => {
                bridge.invoke("CloudExperienceHost.Telemetry.logEvent", "saveOemInfoSuccess", JSON.stringify(telemetryInfos));
                bridge.fireEvent(constants.Events.done, constants.AppResult.success);
            }, (error) => {
                bridge.invoke("CloudExperienceHost.Telemetry.logEvent", "saveOemInfoFailure", core.GetJsonFromError(error));
        ...
        onOemLinkClicked(linkItem, e) {
            if (!this.loadingLink()) {
                ...
                let filePromise = bridge.invoke("CloudExperienceHost.OemRegistration.getLinkFileContent", "",  linkItem.value());
